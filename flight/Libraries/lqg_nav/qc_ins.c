        ...
        float *restrict xnew, float *restrict Pnew)
{
   ...
   const float x13 = H[20]*P[46] + H[21]*P[47] + H[22]*P[48] + H[23]*P[49];
   const float x14 = H[20]*P[47] + H[21]*P[55] + H[22]*P[56] + H[23]*P[57];
   const float x15 = H[20]*P[48] + H[21]*P[56] + H[22]*P[63] + H[23]*P[64];
   const float x16 = H[20]*P[49] + H[21]*P[57] + H[22]*P[64] + H[23]*P[70];
   const float x17 = x13*H[24] + x14*H[25] + x15*H[26] + x16*H[27];
   const float x18 = H[24]*P[46] + H[25]*P[47] + H[26]*P[48] + H[27]*P[49];
   const float x19 = H[24]*P[47] + H[25]*P[55] + H[26]*P[56] + H[27]*P[57];
   const float x20 = H[24]*P[48] + H[25]*P[56] + H[26]*P[63] + H[27]*P[64];
   const float x21 = H[24]*P[49] + H[25]*P[57] + H[26]*P[64] + H[27]*P[70];
   const float x22 = x18*H[28] + x19*H[29] + x20*H[30] + x21*H[31];
   const float x23 = x17*x22;
   const float x24 = x13*H[28] + x14*H[29] + x15*H[30] + x16*H[31];
   const float x25 = x18*H[24] + x19*H[25] + x20*H[26] + x21*H[27] + R[8];
   const float x26 = H[28]*P[46] + H[29]*P[47] + H[30]*P[48] + H[31]*P[49];
   const float x27 = H[28]*P[47] + H[29]*P[55] + H[30]*P[56] + H[31]*P[57];
   const float x28 = H[28]*P[48] + H[29]*P[56] + H[30]*P[63] + H[31]*P[64];
   const float x29 = H[28]*P[49] + H[29]*P[57] + H[30]*P[64] + H[31]*P[70];
   const float x30 = x26*H[20] + x27*H[21] + x28*H[22] + x29*H[23];
   const float x31 = x26*H[24] + x27*H[25] + x28*H[26] + x29*H[27];
   const float x32 = x18*H[20] + x19*H[21] + x20*H[22] + x21*H[23];
   const float x33 = x24*x32;
   const float x34 = x26*H[28] + x27*H[29] + x28*H[30] + x29*H[31] + R[9];
   const float x35 = x17*x32;
   const float x36 = x24*x30;
   const float x37 = x13*H[20] + x14*H[21] + x15*H[22] + x16*H[23] + R[7];
   ...
   const float x47 = 2*param[9];
   const float x48 = x[4]*x[7];
   const float x49 = x[5]*x[6];
   const float x50 = -x9;
   const float x51 = -x0*(x48 - x49) + x47*(x4 + x5) + (x10 + x50 + x8)*param[8] - mag[1];
   ...
   const float x58 = x3*(x48 + x49) - x47*(x1 - x2) + (x11 + x50 + x6 + x7)*param[7] - mag[0];
   ...
   const float x92 = x42 + x44 + x46;
   const float x93 = x53 + x55 + x57;
   const float x94 = x60 + x62 + x64;
   const float x95 = -x92*H[29] - x93*H[25] - x94*H[21];
   const float x96 = -x92*H[30] - x93*H[26] - x94*H[22];
   const float x97 = -x92*H[31] - x93*H[27] - x94*H[23];
   const float x98 = -x92*H[28] - x93*H[24] - x94*H[20] + 1;
   const float x99 = x65 + x66 + x67;
   const float x100 = x68 + x69 + x70;
   const float x101 = x71 + x72 + x73;
   const float x102 = -x100*H[24] - x101*H[20] - x99*H[28];
   const float x103 = -x100*H[26] - x101*H[22] - x99*H[30];
   const float x104 = -x100*H[27] - x101*H[23] - x99*H[31];
   const float x105 = -x100*H[25] - x101*H[21] - x99*H[29] + 1;
   const float x106 = x74 + x75 + x76;
   const float x107 = x77 + x78 + x79;
   const float x108 = x80 + x81 + x82;
   const float x109 = -x106*H[28] - x107*H[24] - x108*H[20];
   const float x110 = -x106*H[29] - x107*H[25] - x108*H[21];
   const float x111 = -x106*H[31] - x107*H[27] - x108*H[23];
   const float x112 = -x106*H[30] - x107*H[26] - x108*H[22] + 1;
   const float x113 = x83 + x84 + x85;
   const float x114 = x86 + x87 + x88;
   const float x115 = x89 + x90 + x91;
   xnew[0] = x[0];
   xnew[1] = x[1];
   xnew[2] = x[2];
   xnew[3] = x[3];
   ...
   Pnew[44] = P[44];
   Pnew[45] = P[45];
   Pnew[46] = x95*P[47] + x96*P[48] + x97*P[49] + x98*P[46];
   Pnew[47] = x95*P[55] + x96*P[56] + x97*P[57] + x98*P[47];
   Pnew[48] = x95*P[56] + x96*P[63] + x97*P[64] + x98*P[48];
   Pnew[49] = x95*P[57] + x96*P[64] + x97*P[70] + x98*P[49];
   Pnew[50] = P[50];
   Pnew[51] = P[51];
   Pnew[52] = P[52];
   Pnew[53] = P[53];
   Pnew[54] = P[54];
   Pnew[55] = x102*P[47] + x103*P[56] + x104*P[57] + x105*P[55];
   Pnew[56] = x102*P[48] + x103*P[63] + x104*P[64] + x105*P[56];
   Pnew[57] = x102*P[49] + x103*P[64] + x104*P[70] + x105*P[57];
   Pnew[58] = P[58];
   Pnew[59] = P[59];
   Pnew[60] = P[60];
   Pnew[61] = P[61];
   Pnew[62] = P[62];
   Pnew[63] = x109*P[48] + x110*P[56] + x111*P[64] + x112*P[63];
   Pnew[64] = x109*P[49] + x110*P[57] + x111*P[70] + x112*P[64];
   Pnew[65] = P[65];
   Pnew[66] = P[66];
   Pnew[67] = P[67];
   Pnew[68] = P[68];
   Pnew[69] = P[69];
   Pnew[70] = (-x113*H[28] - x114*H[24] - x115*H[20])*P[49] + (-x113*H[29] - x114*H[25] - x115*H[21])*P[57] + (-x113*H[30] - x114*H[26] - x115*H[22])*P[64] + (-x113*H[31] - x114*H[27] - x115*H[23] + 1)*P[70];
   Pnew[71] = P[71];
   Pnew[72] = P[72];
   Pnew[73] = P[73];
   Pnew[74] = P[74];
