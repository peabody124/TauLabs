...
void state_prediction(const double * restrict x, const float *u, double Ts, const double *restrict param, double * restrict xnew);
void covariance_prediction(const double *restrict P, double Ts, const double * restrict F, const double * restrict Q, double * restrict Pnew);
void linearize_FH(const double * restrict x, const double * restrict param, double * restrict F, double * restrict H);
void baro_correction(const double *restrict x, const double *restrict P, double baro,
        const double *restrict H, const double *restrict R, double *restrict xnew, double *restrict Pnew);
void mag_correction(const double *restrict x, const double *restrict P, const float *restrict mag,
        const double *restrict H, const double *restrict R, double *restrict xnew, double *restrict Pnew);
void gyro_correction(const double * restrict x, const double * restrict P, const float * restrict gyro,
	const double *restrict H, const double *restrict R,	double *restrict xnew, double *restrict Pnew);
void accel_correction(const double *restrict x, const double *restrict P, const float *restrict accel,
        const double *restrict H, const double *restrict R, const double *restrict param, double *restrict xnew, double *restrict Pnew);
void update_state(const double * restrict x, const double *restrict P, double *restrict xnew, double *restrict Pnew);
...
bool qcins_alloc(uintptr_t *qcins_handle)
{
	...
	qcins_state->params.beta1 = 10000.0f / 180.0 * 3.1415;
...
void baro_correction(const double *restrict x, const double *restrict P, double baro, const double *restrict H,
	                      const double *restrict R, double *restrict xnew, double *restrict Pnew)
 {
...
void mag_correction(const double *restrict x, const double *restrict P, const float *restrict mag,
        const double *restrict H, const double *restrict R, double *restrict xnew, double *restrict Pnew)
{
   const double x0 = -mag[1] - 2*x[4]*x[7] + 2*x[5]*x[6];
   const double x1 = H[19]*P[4] + H[20]*P[5] + H[21]*P[6] + H[22]*P[7];
   const double x2 = H[19]*P[42] + H[20]*P[43] + H[21]*P[44] + H[22]*P[45];
   const double x3 = x2*H[23];
   const double x4 = H[19]*P[43] + H[20]*P[49] + H[21]*P[50] + H[22]*P[51];
   const double x5 = x4*H[24];
   const double x6 = H[19]*P[44] + H[20]*P[50] + H[21]*P[55] + H[22]*P[56];
   const double x7 = x6*H[25];
   const double x8 = H[19]*P[45] + H[20]*P[51] + H[21]*P[56] + H[22]*P[60];
   const double x9 = x8*H[26];
   const double x10 = H[23]*P[42] + H[24]*P[43] + H[25]*P[44] + H[26]*P[45];
   const double x11 = x10*H[19];
   const double x12 = H[23]*P[43] + H[24]*P[49] + H[25]*P[50] + H[26]*P[51];
   const double x13 = x12*H[20];
   const double x14 = H[23]*P[44] + H[24]*P[50] + H[25]*P[55] + H[26]*P[56];
   const double x15 = x14*H[21];
   const double x16 = H[23]*P[45] + H[24]*P[51] + H[25]*P[56] + H[26]*P[60];
   const double x17 = x16*H[22];
   const double x18 = x2*H[19] + x4*H[20] + x6*H[21] + x8*H[22] + R[7];
   const double x19 = x10*H[23] + x12*H[24] + x14*H[25] + x16*H[26] + R[8];
   const double x20 = 1.0F/(x18*x19 - (x11 + x13 + x15 + x17)*(x3 + x5 + x7 + x9));
   const double x21 = x20*(-x3 - x5 - x7 - x9);
   const double x22 = x1*x21;
   const double x23 = H[23]*P[4] + H[24]*P[5] + H[25]*P[6] + H[26]*P[7];
   const double x24 = x18*x20;
   const double x25 = x23*x24;
   const double x26 = -mag[0] + (x[4]*x[4]) + (x[5]*x[5]) - (x[6]*x[6]) - (x[7]*x[7]);
   const double x27 = x20*(-x11 - x13 - x15 - x17);
   const double x28 = x23*x27;
   const double x29 = x19*x20;
   const double x30 = x1*x29;
   const double x31 = H[19]*P[15] + H[20]*P[16] + H[21]*P[17] + H[22]*P[18];
   const double x32 = x21*x31;
   const double x33 = H[23]*P[15] + H[24]*P[16] + H[25]*P[17] + H[26]*P[18];
   const double x34 = x24*x33;
   const double x35 = x27*x33;
   const double x36 = x29*x31;
   const double x37 = H[19]*P[25] + H[20]*P[26] + H[21]*P[27] + H[22]*P[28];
   const double x38 = x21*x37;
   const double x39 = H[23]*P[25] + H[24]*P[26] + H[25]*P[27] + H[26]*P[28];
   const double x40 = x24*x39;
   const double x41 = x27*x39;
   const double x42 = x29*x37;
   const double x43 = H[19]*P[34] + H[20]*P[35] + H[21]*P[36] + H[22]*P[37];
   const double x44 = x21*x43;
   const double x45 = H[23]*P[34] + H[24]*P[35] + H[25]*P[36] + H[26]*P[37];
   const double x46 = x24*x45;
   const double x47 = x27*x45;
   const double x48 = x29*x43;
   const double x49 = x2*x21;
   const double x50 = x10*x24;
   const double x51 = x10*x27;
   const double x52 = x2*x29;
   const double x53 = x21*x4;
   const double x54 = x12*x24;
   const double x55 = x12*x27;
   const double x56 = x29*x4;
   const double x57 = x21*x6;
   const double x58 = x14*x24;
   const double x59 = x14*x27;
   const double x60 = x29*x6;
   const double x61 = x21*x8;
   const double x62 = x16*x24;
   const double x63 = x16*x27;
   const double x64 = x29*x8;
   const double x65 = H[19]*P[46] + H[20]*P[52] + H[21]*P[57] + H[22]*P[61];
   const double x66 = x21*x65;
   const double x67 = H[23]*P[46] + H[24]*P[52] + H[25]*P[57] + H[26]*P[61];
   const double x68 = x24*x67;
   const double x69 = x27*x67;
   const double x70 = x29*x65;
   const double x71 = H[19]*P[47] + H[20]*P[53] + H[21]*P[58] + H[22]*P[62];
   const double x72 = x21*x71;
   const double x73 = H[23]*P[47] + H[24]*P[53] + H[25]*P[58] + H[26]*P[62];
   const double x74 = x24*x73;
   const double x75 = x27*x73;
   const double x76 = x29*x71;
   const double x77 = H[19]*P[48] + H[20]*P[54] + H[21]*P[59] + H[22]*P[63];
   const double x78 = x21*x77;
   const double x79 = H[23]*P[48] + H[24]*P[54] + H[25]*P[59] + H[26]*P[63];
   const double x80 = x24*x79;
   const double x81 = x27*x79;
   const double x82 = x29*x77;
   const double x83 = x22 + x25;
   const double x84 = x28 + x30;
   const double x85 = -x83*H[23] - x84*H[19];
   const double x86 = -x83*H[24] - x84*H[20];
   const double x87 = -x83*H[25] - x84*H[21];
   const double x88 = -x83*H[26] - x84*H[22];
   const double x89 = x32 + x34;
   const double x90 = x35 + x36;
   const double x91 = -x89*H[23] - x90*H[19];
   const double x92 = -x89*H[24] - x90*H[20];
   const double x93 = -x89*H[25] - x90*H[21];
   const double x94 = -x89*H[26] - x90*H[22];
   const double x95 = x38 + x40;
   const double x96 = x41 + x42;
   const double x97 = -x95*H[23] - x96*H[19];
   const double x98 = -x95*H[24] - x96*H[20];
   const double x99 = -x95*H[25] - x96*H[21];
   const double x100 = -x95*H[26] - x96*H[22];
   const double x101 = x44 + x46;
   const double x102 = x47 + x48;
   const double x103 = -x101*H[23] - x102*H[19];
   const double x104 = -x101*H[24] - x102*H[20];
   const double x105 = -x101*H[25] - x102*H[21];
   const double x106 = -x101*H[26] - x102*H[22];
   const double x107 = x49 + x50;
   const double x108 = x51 + x52;
   const double x109 = -x107*H[24] - x108*H[20];
   const double x110 = -x107*H[25] - x108*H[21];
   const double x111 = -x107*H[26] - x108*H[22];
   const double x112 = -x107*H[23] - x108*H[19] + 1;
   const double x113 = x53 + x54;
   const double x114 = x55 + x56;
   const double x115 = -x113*H[23] - x114*H[19];
   const double x116 = -x113*H[25] - x114*H[21];
   const double x117 = -x113*H[26] - x114*H[22];
   const double x118 = -x113*H[24] - x114*H[20] + 1;
   const double x119 = x57 + x58;
   const double x120 = x59 + x60;
   const double x121 = -x119*H[23] - x120*H[19];
   const double x122 = -x119*H[24] - x120*H[20];
   const double x123 = -x119*H[26] - x120*H[22];
   const double x124 = -x119*H[25] - x120*H[21] + 1;
   const double x125 = x61 + x62;
   const double x126 = x63 + x64;
   const double x127 = -x125*H[23] - x126*H[19];
   const double x128 = -x125*H[24] - x126*H[20];
   const double x129 = -x125*H[25] - x126*H[21];
   const double x130 = -x125*H[26] - x126*H[22] + 1;
   const double x131 = x66 + x68;
   const double x132 = x69 + x70;
   const double x133 = x72 + x74;
   const double x134 = x75 + x76;
   const double x135 = x78 + x80;
   const double x136 = x81 + x82;
        ...
        const double *restrict H, const double *restrict R, const double *restrict param, double *restrict xnew, double *restrict Pnew)
{
   ...
   const double x6 = -(x0*(x[4]*x[5] + x[6]*x[7]) - 2*(x1 - x2)*x[1] + (x3 + x4 - x5)*x[2])*param[3] - accel[1];
   ...
   const double x11 = x10 + x8 + x9;
   const double x12 = x11*H[15];
   const double x13 = H[8]*P[22];
   const double x14 = H[9]*P[32];
   const double x15 = H[10]*P[41];
   const double x16 = x13*H[15] + x14*H[15] + x15*H[15];
   const double x17 = x12*x16;
   const double x18 = (H[15]*H[15])*P[73] + R[3];
   const double x19 = H[1]*P[12] + H[2]*P[13] + H[3]*P[14] + H[4]*P[15] + H[5]*P[16] + H[6]*P[17] + H[7]*P[18];
   const double x20 = H[1]*P[13] + H[2]*P[23] + H[3]*P[24] + H[4]*P[25] + H[5]*P[26] + H[6]*P[27] + H[7]*P[28];
   const double x21 = H[1]*P[14] + H[2]*P[24] + H[3]*P[33] + H[4]*P[34] + H[5]*P[35] + H[6]*P[36] + H[7]*P[37];
   const double x22 = H[1]*P[15] + H[2]*P[25] + H[3]*P[34] + H[4]*P[42] + H[5]*P[43] + H[6]*P[44] + H[7]*P[45];
   const double x23 = H[1]*P[16] + H[2]*P[26] + H[3]*P[35] + H[4]*P[43] + H[5]*P[49] + H[6]*P[50] + H[7]*P[51];
   const double x24 = H[1]*P[17] + H[2]*P[27] + H[3]*P[36] + H[4]*P[44] + H[5]*P[50] + H[6]*P[55] + H[7]*P[56];
   const double x25 = H[1]*P[18] + H[2]*P[28] + H[3]*P[37] + H[4]*P[45] + H[5]*P[51] + H[6]*P[56] + H[7]*P[60];
   const double x26 = x19*H[8] + x20*H[9] + x21*H[10] + x22*H[11] + x23*H[12] + x24*H[13] + x25*H[14];
   const double x27 = x18*x26;
   const double x28 = x17 - x27;
   const double x29 = H[8]*P[12] + H[9]*P[13] + H[10]*P[14] + H[11]*P[15] + H[12]*P[16] + H[13]*P[17] + H[14]*P[18];
   const double x30 = H[8]*P[13] + H[9]*P[23] + H[10]*P[24] + H[11]*P[25] + H[12]*P[26] + H[13]*P[27] + H[14]*P[28];
   const double x31 = H[8]*P[14] + H[9]*P[24] + H[10]*P[33] + H[11]*P[34] + H[12]*P[35] + H[13]*P[36] + H[14]*P[37];
   const double x32 = H[8]*P[15] + H[9]*P[25] + H[10]*P[34] + H[11]*P[42] + H[12]*P[43] + H[13]*P[44] + H[14]*P[45];
   const double x33 = H[8]*P[16] + H[9]*P[26] + H[10]*P[35] + H[11]*P[43] + H[12]*P[49] + H[13]*P[50] + H[14]*P[51];
   const double x34 = H[8]*P[17] + H[9]*P[27] + H[10]*P[36] + H[11]*P[44] + H[12]*P[50] + H[13]*P[55] + H[14]*P[56];
   const double x35 = H[8]*P[18] + H[9]*P[28] + H[10]*P[37] + H[11]*P[45] + H[12]*P[51] + H[13]*P[56] + H[14]*P[60];
   const double x36 = x29*H[1] + x30*H[2] + x31*H[3] + x32*H[4] + x33*H[5] + x34*H[6] + x35*H[7];
   const double x37 = x13 + x14 + x15;
   const double x38 = x37*H[15];
   const double x39 = x10*H[15] + x8*H[15] + x9*H[15];
   const double x40 = x38*x39;
   const double x41 = x12*x39;
   const double x42 = x29*H[8] + x30*H[9] + x31*H[10] + x32*H[11] + x33*H[12] + x34*H[13] + x35*H[14] + R[2];
   const double x43 = x16*x38;
   const double x44 = x19*H[1] + x20*H[2] + x21*H[3] + x22*H[4] + x23*H[5] + x24*H[6] + x25*H[7] + R[1];
   const double x45 = x18*x44;
   const double x46 = 1.0F/(x17*x36 + x26*x40 - x27*x36 - x41*x42 + x42*x45 - x43*x44);
   const double x47 = x28*x46;
   const double x48 = x47*x7;
   const double x49 = H[8]*P[1] + H[9]*P[2] + H[10]*P[3] + H[11]*P[4] + H[12]*P[5] + H[13]*P[6] + H[14]*P[7];
   const double x50 = -x41 + x45;
   const double x51 = x46*x50;
   const double x52 = x49*x51;
   const double x53 = x46*(-x16*x44 + x26*x39)*H[15];
   const double x54 = x53*P[11];
   const double x55 = -(-x0*(x[4]*x[6] - x[5]*x[7]) + 2*(x1 + x2)*x[2] + (x3 - x4 + x5)*x[1])*param[3] - accel[0];
   const double x56 = -x18*x36 + x40;
   const double x57 = x46*x56;
   const double x58 = x49*x57;
   const double x59 = x18*x42 - x43;
   const double x60 = x46*x59;
   const double x61 = x60*x7;
   const double x62 = x46*(x16*x36 - x39*x42)*H[15];
   const double x63 = x62*P[11];
   const double x64 = -accel[2] - param[0]*x[14];
   const double x65 = x12*x36 - x38*x44;
   const double x66 = x46*x65;
   const double x67 = x49*x66;
   const double x68 = -x12*x42 + x26*x38;
   const double x69 = x46*x68;
   const double x70 = x69*x7;
   const double x71 = x46*(-x26*x36 + x42*x44)*H[15];
   const double x72 = x71*P[11];
   const double x73 = x19*x47;
   const double x74 = x29*x51;
   const double x75 = x53*P[22];
   const double x76 = x29*x57;
   const double x77 = x19*x60;
   const double x78 = x62*P[22];
   const double x79 = x29*x66;
   const double x80 = x19*x69;
   const double x81 = x71*P[22];
   const double x82 = x20*x47;
   const double x83 = x30*x51;
   const double x84 = x53*P[32];
   const double x85 = x30*x57;
   const double x86 = x20*x60;
   const double x87 = x62*P[32];
   const double x88 = x30*x66;
   const double x89 = x20*x69;
   const double x90 = x71*P[32];
   const double x91 = x21*x47;
   const double x92 = x31*x51;
   const double x93 = x53*P[41];
   const double x94 = x31*x57;
   const double x95 = x21*x60;
   const double x96 = x62*P[41];
   const double x97 = x31*x66;
   const double x98 = x21*x69;
   const double x99 = x71*P[41];
   const double x100 = x22*x47;
   const double x101 = x32*x51;
   const double x102 = x32*x57;
   const double x103 = x22*x60;
   const double x104 = x23*x47;
   const double x105 = x33*x51;
   const double x106 = x33*x57;
   const double x107 = x23*x60;
   const double x108 = x24*x47;
   const double x109 = x34*x51;
   const double x110 = x34*x57;
   const double x111 = x24*x60;
   const double x112 = x25*x47;
   const double x113 = x35*x51;
   const double x114 = x35*x57;
   const double x115 = x25*x60;
   const double x116 = H[1]*P[19] + H[2]*P[29] + H[3]*P[38] + H[4]*P[46] + H[5]*P[52] + H[6]*P[57] + H[7]*P[61];
   const double x117 = x116*x47;
   const double x118 = H[8]*P[19] + H[9]*P[29] + H[10]*P[38] + H[11]*P[46] + H[12]*P[52] + H[13]*P[57] + H[14]*P[61];
   const double x119 = x118*x51;
   const double x120 = x118*x57;
   const double x121 = x116*x60;
   const double x122 = H[1]*P[20] + H[2]*P[30] + H[3]*P[39] + H[4]*P[47] + H[5]*P[53] + H[6]*P[58] + H[7]*P[62];
   const double x123 = x122*x47;
   const double x124 = H[8]*P[20] + H[9]*P[30] + H[10]*P[39] + H[11]*P[47] + H[12]*P[53] + H[13]*P[58] + H[14]*P[62];
   const double x125 = x124*x51;
   const double x126 = x124*x57;
   const double x127 = x122*x60;
   const double x128 = H[1]*P[21] + H[2]*P[31] + H[3]*P[40] + H[4]*P[48] + H[5]*P[54] + H[6]*P[59] + H[7]*P[63];
   const double x129 = x128*x47;
   const double x130 = H[8]*P[21] + H[9]*P[31] + H[10]*P[40] + H[11]*P[48] + H[12]*P[54] + H[13]*P[59] + H[14]*P[63];
   const double x131 = x130*x51;
   const double x132 = x130*x57;
   const double x133 = x128*x60;
   const double x134 = x11*x46;
   const double x135 = x134*x28;
   const double x136 = x37*x46;
   const double x137 = x136*x50;
   const double x138 = x53*P[73];
   const double x139 = x136*x56;
   const double x140 = x134*x59;
   const double x141 = x62*P[73];
   const double x142 = x136*x65;
   const double x143 = x134*x68;
   const double x144 = x71*P[73];
   const double x145 = (x67 + x70 + x72)*H[15];
   const double x146 = x48 + x52 + x54;
   const double x147 = x58 + x61 + x63;
   const double x148 = -x146*H[8] - x147*H[1];
   const double x149 = -x146*H[9] - x147*H[2];
   const double x150 = -x146*H[10] - x147*H[3];
   const double x151 = -x146*H[11] - x147*H[4];
   const double x152 = -x146*H[12] - x147*H[5];
   const double x153 = -x146*H[13] - x147*H[6];
   const double x154 = -x146*H[14] - x147*H[7];
   const double x155 = (x79 + x80 + x81)*H[15];
   const double x156 = x73 + x74 + x75;
   const double x157 = x76 + x77 + x78;
   const double x158 = -x156*H[9] - x157*H[2];
   const double x159 = -x156*H[10] - x157*H[3];
   const double x160 = -x156*H[11] - x157*H[4];
   const double x161 = -x156*H[12] - x157*H[5];
   const double x162 = -x156*H[13] - x157*H[6];
   const double x163 = -x156*H[14] - x157*H[7];
   const double x164 = -x156*H[8] - x157*H[1] + 1;
   const double x165 = (x88 + x89 + x90)*H[15];
   const double x166 = x82 + x83 + x84;
   const double x167 = x85 + x86 + x87;
   const double x168 = -x166*H[8] - x167*H[1];
   const double x169 = -x166*H[10] - x167*H[3];
   const double x170 = -x166*H[11] - x167*H[4];
   const double x171 = -x166*H[12] - x167*H[5];
   const double x172 = -x166*H[13] - x167*H[6];
   const double x173 = -x166*H[14] - x167*H[7];
   const double x174 = -x166*H[9] - x167*H[2] + 1;
   const double x175 = (x97 + x98 + x99)*H[15];
   const double x176 = x91 + x92 + x93;
   const double x177 = x94 + x95 + x96;
   const double x178 = -x176*H[8] - x177*H[1];
   const double x179 = -x176*H[9] - x177*H[2];
   const double x180 = -x176*H[11] - x177*H[4];
   const double x181 = -x176*H[12] - x177*H[5];
   const double x182 = -x176*H[13] - x177*H[6];
   const double x183 = -x176*H[14] - x177*H[7];
   const double x184 = -x176*H[10] - x177*H[3] + 1;
   const double x185 = x100 + x101;
   const double x186 = x102 + x103;
   const double x187 = -x185*H[8] - x186*H[1];
   const double x188 = -x185*H[9] - x186*H[2];
   const double x189 = -x185*H[10] - x186*H[3];
   const double x190 = -x185*H[12] - x186*H[5];
   const double x191 = -x185*H[13] - x186*H[6];
   const double x192 = -x185*H[14] - x186*H[7];
   const double x193 = -x185*H[11] - x186*H[4] + 1;
   const double x194 = x104 + x105;
   const double x195 = x106 + x107;
   const double x196 = -x194*H[8] - x195*H[1];
   const double x197 = -x194*H[9] - x195*H[2];
   const double x198 = -x194*H[10] - x195*H[3];
   const double x199 = -x194*H[11] - x195*H[4];
   const double x200 = -x194*H[13] - x195*H[6];
   const double x201 = -x194*H[14] - x195*H[7];
   const double x202 = -x194*H[12] - x195*H[5] + 1;
   const double x203 = x108 + x109;
   const double x204 = x110 + x111;
   const double x205 = -x203*H[8] - x204*H[1];
   const double x206 = -x203*H[9] - x204*H[2];
   const double x207 = -x203*H[10] - x204*H[3];
   const double x208 = -x203*H[11] - x204*H[4];
   const double x209 = -x203*H[12] - x204*H[5];
   const double x210 = -x203*H[14] - x204*H[7];
   const double x211 = -x203*H[13] - x204*H[6] + 1;
   const double x212 = x112 + x113;
   const double x213 = x114 + x115;
   const double x214 = -x212*H[8] - x213*H[1];
   const double x215 = -x212*H[9] - x213*H[2];
   const double x216 = -x212*H[10] - x213*H[3];
   const double x217 = -x212*H[11] - x213*H[4];
   const double x218 = -x212*H[12] - x213*H[5];
   const double x219 = -x212*H[13] - x213*H[6];
   const double x220 = -x212*H[14] - x213*H[7] + 1;
   const double x221 = x117 + x119;
   const double x222 = x120 + x121;
   const double x223 = x123 + x125;
   const double x224 = x126 + x127;
   const double x225 = x129 + x131;
   const double x226 = x132 + x133;
   const double x227 = x135 + x137 + x138;
   const double x228 = x139 + x140 + x141;
