...
void linearize_FH(const float * restrict x, const float Ts, const float * restrict param, float * restrict F, float * restrict H)
{
   ...
   const float x57 = Ts*param[0];
   const float x58 = Ts*(-x14*x22 - x15*x26);
   const float x59 = x14*param[3];
   const float x60 = x53 - x55 - x56;
   const float x61 = (1.0F/2.0F)*Ts;
   const float x62 = x61*x[8];
   const float x63 = -x62;
   const float x64 = x61*x[9];
   const float x65 = -x64;
   const float x66 = x61*x[10];
   const float x67 = -x66;
   const float x68 = x61*x[5];
   const float x69 = -x68;
   const float x70 = x61*x[6];
   const float x71 = -x70;
   const float x72 = x61*x[7];
   const float x73 = -x72;
   const float x74 = x61*x[4];
   const float x75 = Ts*param[1];
   const float x76 = -Ts/param[2];
   const float x77 = -x46*param[3];
   const float x78 = -x29*param[3];
   const float x79 = -x28;
   F[0] = Ts;
   F[1] = Ts*(-(x11*x11)*param[3] - (x4*x4)*param[3]);
   F[2] = x17;
   F[3] = x27;
   F[4] = Ts*(-x12*x29 - x33*x34 + x41);
   F[5] = Ts*(-x12*x45 - x34*x46 + x47);
   F[6] = Ts*(-x12*x46 - x34*x48 + x52);
   F[7] = Ts*(-x12*x54 - x29*x34 - x53 + x55 + x56);
   F[8] = -x57*(x23 + x24);
   F[9] = x17;
   F[10] = Ts*(-(x14*x14)*param[3] - (x15*x15)*param[3]);
   F[11] = x58;
   F[12] = Ts*(-x16*x33 - x29*x59 + x60);
   F[13] = Ts*(-x16*x46 - x45*x59 + x49 + x50 - x51);
   F[14] = Ts*(-x16*x48 - x46*x59 + x47);
   F[15] = Ts*(-x16*x29 + x41 - x54*x59);
   F[16] = -x57*(-x18 + x20);
   F[17] = x27;
   F[18] = x58;
   F[19] = Ts*(-(x21*x21)*param[3] - (x25*x25)*param[3]);
   F[20] = Ts*(-x22*x29 - x26*x33 + x52);
   F[21] = Ts*(-x22*x45 - x26*x46 + x60);
   F[22] = Ts*(-x22*x46 - x26*x48 + x36 - x38 + x40);
   F[23] = Ts*(-x22*x54 - x26*x29 + x47);
   F[24] = -x57*(x13 + x8 + x9);
   F[25] = x63;
   F[26] = x65;
   F[27] = x67;
...
void covariance_prediction(const float *restrict P, const float * restrict F, const float * restrict Q, float * restrict Pnew)
{
 const float x0 = F[0]*P[33] + P[3];
   ...
   const float x14 = F[55] + 1;
   const float x15 = x7*P[22] + F[2]*P[32] + F[3]*P[41] + F[8]*P[73];
   const float x16 = x7*P[15] + F[2]*P[25] + F[3]*P[34] + F[4]*P[42] + F[5]*P[43] + F[6]*P[44] + F[7]*P[45];
   const float x17 = x7*P[16] + F[2]*P[26] + F[3]*P[35] + F[4]*P[43] + F[5]*P[49] + F[6]*P[50] + F[7]*P[51];
   const float x18 = x7*P[17] + F[2]*P[27] + F[3]*P[36] + F[4]*P[44] + F[5]*P[50] + F[6]*P[55] + F[7]*P[56];
   const float x19 = x7*P[18] + F[2]*P[28] + F[3]*P[37] + F[4]*P[45] + F[5]*P[51] + F[6]*P[56] + F[7]*P[60];
   const float x20 = x7*P[13] + F[2]*P[23] + F[3]*P[24] + F[4]*P[25] + F[5]*P[26] + F[6]*P[27] + F[7]*P[28] + F[8]*P[32];
   const float x21 = x7*P[14] + F[2]*P[24] + F[3]*P[33] + F[4]*P[34] + F[5]*P[35] + F[6]*P[36] + F[7]*P[37] + F[8]*P[41];
   const float x22 = x7*P[12] + F[2]*P[13] + F[3]*P[14] + F[4]*P[15] + F[5]*P[16] + F[6]*P[17] + F[7]*P[18] + F[8]*P[22];
   const float x23 = x7*P[19] + F[2]*P[29] + F[3]*P[38] + F[4]*P[46] + F[5]*P[52] + F[6]*P[57] + F[7]*P[61];
   const float x24 = x7*P[20] + F[2]*P[30] + F[3]*P[39] + F[4]*P[47] + F[5]*P[53] + F[6]*P[58] + F[7]*P[62];
   const float x25 = x7*P[21] + F[2]*P[31] + F[3]*P[40] + F[4]*P[48] + F[5]*P[54] + F[6]*P[59] + F[7]*P[63];
   const float x26 = x9*P[32] + F[9]*P[22] + F[11]*P[41] + F[16]*P[73];
   const float x27 = x9*P[25] + F[9]*P[15] + F[11]*P[34] + F[12]*P[42] + F[13]*P[43] + F[14]*P[44] + F[15]*P[45];
   const float x28 = x9*P[26] + F[9]*P[16] + F[11]*P[35] + F[12]*P[43] + F[13]*P[49] + F[14]*P[50] + F[15]*P[51];
   const float x29 = x9*P[27] + F[9]*P[17] + F[11]*P[36] + F[12]*P[44] + F[13]*P[50] + F[14]*P[55] + F[15]*P[56];
   const float x30 = x9*P[28] + F[9]*P[18] + F[11]*P[37] + F[12]*P[45] + F[13]*P[51] + F[14]*P[56] + F[15]*P[60];
   const float x31 = x9*P[13] + F[9]*P[12] + F[11]*P[14] + F[12]*P[15] + F[13]*P[16] + F[14]*P[17] + F[15]*P[18] + F[16]*P[22];
   const float x32 = x9*P[24] + F[9]*P[14] + F[11]*P[33] + F[12]*P[34] + F[13]*P[35] + F[14]*P[36] + F[15]*P[37] + F[16]*P[41];
   const float x33 = x9*P[23] + F[9]*P[13] + F[11]*P[24] + F[12]*P[25] + F[13]*P[26] + F[14]*P[27] + F[15]*P[28] + F[16]*P[32];
   const float x34 = x9*P[29] + F[9]*P[19] + F[11]*P[38] + F[12]*P[46] + F[13]*P[52] + F[14]*P[57] + F[15]*P[61];
   const float x35 = x9*P[30] + F[9]*P[20] + F[11]*P[39] + F[12]*P[47] + F[13]*P[53] + F[14]*P[58] + F[15]*P[62];
   const float x36 = x9*P[31] + F[9]*P[21] + F[11]*P[40] + F[12]*P[48] + F[13]*P[54] + F[14]*P[59] + F[15]*P[63];
   const float x37 = x10*P[41] + F[17]*P[22] + F[18]*P[32] + F[24]*P[73];
   const float x38 = x10*P[34] + F[17]*P[15] + F[18]*P[25] + F[20]*P[42] + F[21]*P[43] + F[22]*P[44] + F[23]*P[45];
   const float x39 = x10*P[35] + F[17]*P[16] + F[18]*P[26] + F[20]*P[43] + F[21]*P[49] + F[22]*P[50] + F[23]*P[51];
   const float x40 = x10*P[36] + F[17]*P[17] + F[18]*P[27] + F[20]*P[44] + F[21]*P[50] + F[22]*P[55] + F[23]*P[56];
   const float x41 = x10*P[37] + F[17]*P[18] + F[18]*P[28] + F[20]*P[45] + F[21]*P[51] + F[22]*P[56] + F[23]*P[60];
   const float x42 = x10*P[38] + F[17]*P[19] + F[18]*P[29] + F[20]*P[46] + F[21]*P[52] + F[22]*P[57] + F[23]*P[61];
   const float x43 = x10*P[39] + F[17]*P[20] + F[18]*P[30] + F[20]*P[47] + F[21]*P[53] + F[22]*P[58] + F[23]*P[62];
   const float x44 = x10*P[40] + F[17]*P[21] + F[18]*P[31] + F[20]*P[48] + F[21]*P[54] + F[22]*P[59] + F[23]*P[63];
   const float x45 = F[25]*P[43] + F[26]*P[44] + F[27]*P[45] + F[28]*P[46] + F[29]*P[47] + F[30]*P[48] + P[42];
   const float x46 = F[25]*P[52] + F[26]*P[57] + F[27]*P[61] + F[28]*P[64] + P[46];
   const float x47 = F[25]*P[53] + F[26]*P[58] + F[27]*P[62] + F[29]*P[66] + P[47];
   const float x48 = F[25]*P[54] + F[26]*P[59] + F[27]*P[63] + F[30]*P[68] + P[48];
   const float x49 = F[25]*P[49] + F[26]*P[50] + F[27]*P[51] + F[28]*P[52] + F[29]*P[53] + F[30]*P[54] + P[43];
   const float x50 = F[25]*P[50] + F[26]*P[55] + F[27]*P[56] + F[28]*P[57] + F[29]*P[58] + F[30]*P[59] + P[44];
   const float x51 = F[25]*P[51] + F[26]*P[56] + F[27]*P[60] + F[28]*P[61] + F[29]*P[62] + F[30]*P[63] + P[45];
   const float x52 = F[49]*P[65];
   const float x53 = F[50]*P[67];
   const float x54 = F[51]*P[69];
   const float x55 = F[31]*P[43] + F[32]*P[50] + F[33]*P[51] + F[34]*P[52] + F[35]*P[53] + F[36]*P[54] + P[49];
   const float x56 = F[31]*P[46] + F[32]*P[57] + F[33]*P[61] + F[34]*P[64] + P[52];
   const float x57 = F[31]*P[47] + F[32]*P[58] + F[33]*P[62] + F[35]*P[66] + P[53];
   const float x58 = F[31]*P[48] + F[32]*P[59] + F[33]*P[63] + F[36]*P[68] + P[54];
   const float x59 = F[31]*P[42] + F[32]*P[44] + F[33]*P[45] + F[34]*P[46] + F[35]*P[47] + F[36]*P[48] + P[43];
   const float x60 = F[31]*P[44] + F[32]*P[55] + F[33]*P[56] + F[34]*P[57] + F[35]*P[58] + F[36]*P[59] + P[50];
   const float x61 = F[31]*P[45] + F[32]*P[56] + F[33]*P[60] + F[34]*P[61] + F[35]*P[62] + F[36]*P[63] + P[51];
   const float x62 = F[37]*P[44] + F[38]*P[50] + F[39]*P[56] + F[40]*P[57] + F[41]*P[58] + F[42]*P[59] + P[55];
   const float x63 = F[37]*P[46] + F[38]*P[52] + F[39]*P[61] + F[40]*P[64] + P[57];
   const float x64 = F[37]*P[47] + F[38]*P[53] + F[39]*P[62] + F[41]*P[66] + P[58];
   const float x65 = F[37]*P[48] + F[38]*P[54] + F[39]*P[63] + F[42]*P[68] + P[59];
   const float x66 = F[37]*P[42] + F[38]*P[43] + F[39]*P[45] + F[40]*P[46] + F[41]*P[47] + F[42]*P[48] + P[44];
   const float x67 = F[37]*P[43] + F[38]*P[49] + F[39]*P[51] + F[40]*P[52] + F[41]*P[53] + F[42]*P[54] + P[50];
   const float x68 = F[37]*P[45] + F[38]*P[51] + F[39]*P[60] + F[40]*P[61] + F[41]*P[62] + F[42]*P[63] + P[56];
   const float x69 = F[43]*P[46] + F[44]*P[52] + F[45]*P[57] + F[46]*P[64] + P[61];
   const float x70 = F[43]*P[47] + F[44]*P[53] + F[45]*P[58] + F[47]*P[66] + P[62];
   const float x71 = F[43]*P[48] + F[44]*P[54] + F[45]*P[59] + F[48]*P[68] + P[63];
   const float x72 = F[49]*P[70] + P[65];
   const float x73 = F[52] + 1;
   const float x74 = F[50]*P[71] + P[67];
   const float x75 = F[53] + 1;
   const float x76 = F[51]*P[72] + P[69];
   const float x77 = F[54] + 1;
   ...
   Pnew[10] = x13;
   Pnew[11] = x14*x6;
   Pnew[12] = x15*F[8] + x16*F[4] + x17*F[5] + x18*F[6] + x19*F[7] + x20*F[2] + x21*F[3] + x22*x7 + Q[1];
   Pnew[13] = x15*F[16] + x16*F[12] + x17*F[13] + x18*F[14] + x19*F[15] + x20*x9 + x21*F[11] + x22*F[9];
   Pnew[14] = x10*x21 + x15*F[24] + x16*F[20] + x17*F[21] + x18*F[22] + x19*F[23] + x20*F[18] + x22*F[17];
   Pnew[15] = x16 + x17*F[25] + x18*F[26] + x19*F[27] + x23*F[28] + x24*F[29] + x25*F[30];
   Pnew[16] = x16*F[31] + x17 + x18*F[32] + x19*F[33] + x23*F[34] + x24*F[35] + x25*F[36];
   Pnew[17] = x16*F[37] + x17*F[38] + x18 + x19*F[39] + x23*F[40] + x24*F[41] + x25*F[42];
   Pnew[18] = x16*F[43] + x17*F[44] + x18*F[45] + x19 + x23*F[46] + x24*F[47] + x25*F[48];
   Pnew[19] = x23;
   Pnew[20] = x24;
   Pnew[21] = x25;
   Pnew[22] = x14*x15;
   Pnew[23] = x26*F[16] + x27*F[12] + x28*F[13] + x29*F[14] + x30*F[15] + x31*F[9] + x32*F[11] + x33*x9 + Q[2];
   Pnew[24] = x10*x32 + x26*F[24] + x27*F[20] + x28*F[21] + x29*F[22] + x30*F[23] + x31*F[17] + x33*F[18];
   Pnew[25] = x27 + x28*F[25] + x29*F[26] + x30*F[27] + x34*F[28] + x35*F[29] + x36*F[30];
   Pnew[26] = x27*F[31] + x28 + x29*F[32] + x30*F[33] + x34*F[34] + x35*F[35] + x36*F[36];
   Pnew[27] = x27*F[37] + x28*F[38] + x29 + x30*F[39] + x34*F[40] + x35*F[41] + x36*F[42];
   Pnew[28] = x27*F[43] + x28*F[44] + x29*F[45] + x30 + x34*F[46] + x35*F[47] + x36*F[48];
   Pnew[29] = x34;
   Pnew[30] = x35;
   Pnew[31] = x36;
   Pnew[32] = x14*x26;
   Pnew[33] = x10*(x10*P[33] + F[17]*P[14] + F[18]*P[24] + F[20]*P[34] + F[21]*P[35] + F[22]*P[36] + F[23]*P[37] + F[24]*P[41]) + x37*F[24] + x38*F[20] + x39*F[21] + x40*F[22] + x41*F[23] + (x10*P[14] + F[17]*P[12] + F[18]*P[13] + F[20]*P[15] + F[21]*P[16] + F[22]*P[17] + F[23]*P[18] + F[24]*P[22])*F[17] + (x10*P[24] + F[17]*P[13] + F[18]*P[23] + F[20]*P[25] + F[21]*P[26] + F[22]*P[27] + F[23]*P[28] + F[24]*P[32])*F[18] + Q[3];
   Pnew[34] = x38 + x39*F[25] + x40*F[26] + x41*F[27] + x42*F[28] + x43*F[29] + x44*F[30];
   Pnew[35] = x38*F[31] + x39 + x40*F[32] + x41*F[33] + x42*F[34] + x43*F[35] + x44*F[36];
   Pnew[36] = x38*F[37] + x39*F[38] + x40 + x41*F[39] + x42*F[40] + x43*F[41] + x44*F[42];
   Pnew[37] = x38*F[43] + x39*F[44] + x40*F[45] + x41 + x42*F[46] + x43*F[47] + x44*F[48];
   Pnew[38] = x42;
   Pnew[39] = x43;
   Pnew[40] = x44;
   Pnew[41] = x14*x37;
   ...
   Pnew[47] = x47 + x53*F[29];
   Pnew[48] = x48 + x54*F[30];
   Pnew[49] = x55 + x56*F[34] + x57*F[35] + x58*F[36] + x59*F[31] + x60*F[32] + x61*F[33] + Q[5];
   Pnew[50] = x55*F[38] + x56*F[40] + x57*F[41] + x58*F[42] + x59*F[37] + x60 + x61*F[39];
   Pnew[51] = x55*F[44] + x56*F[46] + x57*F[47] + x58*F[48] + x59*F[43] + x60*F[45] + x61;
   Pnew[52] = x52*F[34] + x56;
   Pnew[53] = x53*F[35] + x57;
   Pnew[54] = x54*F[36] + x58;
   Pnew[55] = x62 + x63*F[40] + x64*F[41] + x65*F[42] + x66*F[37] + x67*F[38] + x68*F[39] + Q[6];
   Pnew[56] = x62*F[45] + x63*F[46] + x64*F[47] + x65*F[48] + x66*F[43] + x67*F[44] + x68;
   Pnew[57] = x52*F[40] + x63;
   Pnew[58] = x53*F[41] + x64;
   Pnew[59] = x54*F[42] + x65;
   Pnew[60] = x69*F[46] + x70*F[47] + x71*F[48] + (F[43]*P[42] + F[44]*P[43] + F[45]*P[44] + F[46]*P[46] + F[47]*P[47] + F[48]*P[48] + P[45])*F[43] + (F[43]*P[43] + F[44]*P[49] + F[45]*P[50] + F[46]*P[52] + F[47]*P[53] + F[48]*P[54] + P[51])*F[44] + (F[43]*P[44] + F[44]*P[50] + F[45]*P[55] + F[46]*P[57] + F[47]*P[58] + F[48]*P[59] + P[56])*F[45] + F[43]*P[45] + F[44]*P[51] + F[45]*P[56] + F[46]*P[61] + F[47]*P[62] + F[48]*P[63] + P[60] + Q[7];
   Pnew[61] = x52*F[46] + x69;
   Pnew[62] = x53*F[47] + x70;
   Pnew[63] = x54*F[48] + x71;
   Pnew[64] = x52 + x72*F[49] + P[64] + Q[8];
   Pnew[65] = x72*x73;
   Pnew[66] = x53 + x74*F[50] + P[66] + Q[9];
   Pnew[67] = x74*x75;
   Pnew[68] = x54 + x76*F[51] + P[68] + Q[10];
   Pnew[69] = x76*x77;
   Pnew[70] = (x73*x73)*P[70] + Q[11];
   Pnew[71] = (x75*x75)*P[71] + Q[12];
   Pnew[72] = (x77*x77)*P[72] + Q[13];
   Pnew[73] = (x14*x14)*P[73] + Q[14];
}
